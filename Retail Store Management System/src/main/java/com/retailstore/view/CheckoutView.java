...
public class CheckoutView extends JFrame {
    ...
    public CheckoutView() {
        ...
        headerLabel.setFont(new Font(config.getDefaultFont(), Font.BOLD, config.getTitleFontSize()));
    ...
    private JPanel createCostPanel() {
        ...
        // Set up fonts
        Font regularFont = new Font(config.getDefaultFont(), Font.PLAIN, config.getDefaultFontSize() + 4);
        Font boldFont = new Font(config.getDefaultFont(), Font.BOLD, config.getDefaultFontSize() + 6);

        // Configure labels with larger fonts and tax rate from config
        labSubtotal.setFont(regularFont);
        labSubtotal.setHorizontalAlignment(SwingConstants.RIGHT);

        labTax.setText("Tax (" + (int)(config.getTaxRate() * 100) + "%): $0.00");
    ...
    private JPanel createButtonPanel() {
        ...
        // Set fonts
        Font buttonFont = new Font(config.getDefaultFont(), Font.BOLD, config.getDefaultFontSize() + 2);
    ...
    private void setupTable() {
        ...
        // Set table font
        tblItems.setFont(new Font(config.getDefaultFont(), Font.PLAIN, config.getDefaultFontSize() + 2));
        tblItems.getTableHeader().setFont(new Font(config.getDefaultFont(), Font.BOLD, config.getDefaultFontSize() + 2));
    }

    public void updateCosts(double subtotal) {
        // Calculate tax and total using config tax rate
